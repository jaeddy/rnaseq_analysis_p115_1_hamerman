...
might come in handy with some steps of the analysis. I've modified the function
slightly, such that library sizes are updated and normalization factors are
calculated *after* filtering genes. I've also added the option to input gene
...
Keeping only those genes with >= 10 counts per million in at least 10% (`r 0.2 
...
I'll perform PCA with the `prcomp` package and plot the first two principal
...
***Experimental, sequencing, and alingment variables plotted against the 1st
principal component***
...
```{r group_fit_model}
# Estimate correlation for samples from the same HSC population
hscCor <- duplicateCorrelation(koVoom, koDesign, block = groupDat$hscPop)

# Fit model for the group design
koFit <- lmFit(koVoom, koDesign, block = groupDat$hscPop,
               correlation = hscCor$consensus.correlation)
koFit <- eBayes(koFit)
