...
```{r plot_ko_status, echo=FALSE, fig.height=8}
# Plot PC1 against all other variables
confoundDat %>% 
...
This model produces `nrow(koResults %>% filter(adj.P.Val < 0.05))` significant 
genes:
`kable(koResults %>% filter(adj.P.Val < 0.05), format = "html")`
...
`kable(koResults %>% filter(mgiSymbol == "Pik3ap1"), format = "html")`
...
This model produces `nrow(koHscResults %>% filter(adj.P.Val < 0.05))` 
significant genes for `koStatus`:
`kable(koHscResults %>% filter(adj.P.Val < 0.05), format = "html")`

**Pik3ap1**, is still not significant after multiple testing correction:
`kable(koHscResults %>% filter(mgiSymbol == "Pik3ap1"), format = "html")`
...
data_frame(pc1 = pca$x[, 1], sv1 = koSva$sv[, 1], 
           voomQWts = koHscVoom$sample.weights,
...
This new model produces `nrow(koHscConcResults %>% filter(adj.P.Val < 0.05))` 
significant genes for `koStatus`:
`kable(koHscConcResults %>% filter(adj.P.Val < 0.05), format = "html")`

**Pik3ap1**, is still not significant after multiple testing correction:
`kable(koHscConcResults %>% filter(mgiSymbol == "Pik3ap1"), format = "html")`
...
# Define the design matrix, including terms corresponding to KO status and HSC
# population; use voom to calculate transformed expression values
koHscConcVoomNoQual <- voom(dge, design = koHscDesign,
                                    plot = TRUE)
```
...
`nrow(koHscConcNoQualResults %>% filter(adj.P.Val < 0.05))` 
significant genes for `koStatus`:
`kable(koHscConcNoQualResults %>% filter(adj.P.Val < 0.05), format = "html")`

**Pik3ap1**, is borderline significant after multiple testing correction:
`kable(koHscConcNoQualResults %>% filter(mgiSymbol == "Pik3ap1"), format = "html")`
...
```{r plot_bcap_fits, echo=FALSE, fig.height=4.5}
# Pull out raw counts for BCAP gene (Pik3ap1)
bcapIdx <- which(koVoom$genes$mgiSymbol == "Pik3ap1")
bcapHscIdx <- which(koHscVoom$genes$mgiSymbol == "Pik3ap1")
...
bcapDatAll <- koVoom$E[bcapIdx, ] %>% 
    ...
    mutate(koStatus = groupDat$koStatus,
           hscPop = groupDat$hscPop,
           samples = "all",
           fit = fitted(koFit)[bcapIdx, ]) %>% 
    bind_rows(koHscVoom$E[bcapHscIdx, ] %>% 
                  melt(value.name = "logCPM") %>% 
                  add_rownames(var = "libID") %>% 
                  mutate(koStatus = groupDat$koStatus,
                         hscPop = groupDat$hscPop,
                         samples = "all_plusHsc",
                         fit = fitted(koHscFit)[bcapHscIdx, ])) %>% 
    bind_rows(koVoomShort$E[bcapIdxShort, ] %>% 
                  melt(value.name = "logCPM") %>% 
                  add_rownames(var = "libID") %>% 
                  mutate(koStatus = groupDatShort$koStatus,
                         hscPop = groupDatShort$hscPop,
                         samples = "short",
                         fit = fitted(koFitShort)[bcapIdxShort, ])) %>% 
    bind_rows(koVoomLong$E[bcapIdxLong, ] %>% 
                  melt(value.name = "logCPM") %>% 
                  add_rownames(var = "libID") %>% 
                  mutate(koStatus = groupDatLong$koStatus,
                         hscPop = groupDatLong$hscPop,
                         samples = "long",
                         fit = fitted(koFitLong)[bcapIdxLong, ]))
...
bcapDatAll %>% 
    ...
    geom_errorbar(stat = "hline", yintercept = "mean",
                  width = 0.8, aes(ymin = fit, ymax = fit)) +
    facet_wrap(~ samples) +
    theme_classic() +
    scale_color_colorblind() + 
    scale_fill_colorblind() +
